...
const TOKEN_KEY = 'toursales_vendor_token';
const REFRESH_TOKEN_KEY = 'toursales_vendor_refresh_token';
...
export const vendorStorage = {
  ...
  clear: (): void => {
    ...
    localStorage.removeItem('toursales_vendor_user');
  },
};
...
export const vendorAxiosClient = axios.create({
...
vendorAxiosClient.interceptors.request.use(
  (config: InternalAxiosRequestConfig) => {
    if (config.url && config.url.startsWith('/api/v1')) {
      config.url = config.url.substring('/api/v1'.length);
    }
    const token = vendorStorage.getToken();
...
vendorAxiosClient.interceptors.response.use(
  ...
  async (error: AxiosError) => {
    ...
    if (error.response?.status === 401 && !isAuthRoute && !originalRequest?._retry) {
      const refreshToken = vendorStorage.getRefreshToken();
      if (!refreshToken) {
        vendorStorage.clear();
      ...
      try {
        ...
        if (newToken) {
          vendorStorage.setToken(newToken);
          if (newRefreshToken) {
            vendorStorage.setRefreshToken(newRefreshToken);
          }
          if (originalRequest.headers) {
